...
import { Convert, getCurriencies } from '../Components/Data/Curriencies';
  ...
  const [selectedValue, setSelectedValue] = useState([]);
   ...
   const Selected =(code)=>{
    ...
    const handleClickOpen = () => {
      ...
      const setCode = (code) =>{ 
        alert("hahuwa l code  tle3 l parent : "+code) 
        Convert(code,10).then(resp => { console.log(resp)})
      }
    ...
    return (
        ...
        <div className="submitBtxnCon">
          ...
          {
            list.map( item  => (
               item.popular  &&  ( <LogoCard key={item.id} setCode={setCode}  code ={item.code}   title={item.name} />)
            ))}
         
            <button className="hiddenBtn" s  onClick={handleClickOpen} >     
        ...
        <span>You will receive  Kishu for {} </span>
